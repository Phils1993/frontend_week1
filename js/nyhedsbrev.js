...
function populateNyhedsbreve() {
    ...
    nyhedsbreve.slice(0, 7).forEach(nyhedsbrev => {
        const listItem = document.createElement('li');
        listItem.innerHTML = `<a href="${nyhedsbrev.link}" target="_blank">${nyhedsbrev.title}</a>`;
        dropdownMenu.appendChild(listItem);
    });

    // "Vis mere"-knap, hvis der er flere end 7 nyhedsbreve
    if (nyhedsbreve.length > 7) {
        const visMere = document.createElement('li');
        visMere.innerHTML = `<a href="alle-nyhedsbreve.html">Vis mere</a>`;
